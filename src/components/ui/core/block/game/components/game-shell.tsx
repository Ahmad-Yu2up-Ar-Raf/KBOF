  ...
  return (
    <>
      <motion.header
        ...
        className={cn(
          // Fixed on small screens (works with Lenis), sticky on md+
          'z-40  bg-background fixed md:sticky top-0 left-0 right-0 mx-auto md:bg-transparent pt-5 items-center justify-center flex text-center w-full px-10 h-fit',
          className,
          isColumn && ' pb-3',
        )}
      >
        ...
        {leftAction && (
          <nav
            className={cn(
              'z-50 absolute left-5 md:left-0  bg-none ',
          ...
          >
            {isLeftActionFunction ? (
              <Button
                ...
                variant="ghost"
                size="icon"
                className="flex bg-none bg-transparent items-center w-fit py-0 text-base items-center gap-2 group transition-colors"
              >
        ...
        <div className="z-50 absolute  md:right-0 right-5 top-1/2 -translate-y-1/2   ">
          {rightAction}
        </div>
      </motion.header>

      {/* spacer for fixed header on small screens to avoid content overlap */}
      {isColumn && <div className="md:hidden h-16" aria-hidden="true" />}
    </>
  )
}
