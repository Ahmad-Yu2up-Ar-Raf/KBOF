...
export default function HeroSection() {
  ...
  if (isMobile) return <HeroSectionMobile />
...
export function HeroSectionMobile() {
  ...
  const style = { width: 250, height: 150, margin: 'auto' }
  const { View } = useLottie(lottieOptions, style)
  const { data: session } = authClient.useSession()
  ...
  return (
    <section className="w-full h-full">
      <BlurFade
        ...
        delay={1}
        className="w-full m-auto mb-0.5 overflow-hidden md:mb-25 lg:mb-40 lg:pt-20 min-h-svh md:overflow-visible flex flex-col items-center justify-center relative"
      >
        {/* Floating images - visible on all devices */}
        <Floating sensitivity={-0.5} className="h-full">
          <FloatingElement depth={1} className="top-[0%] left-[3%]">
