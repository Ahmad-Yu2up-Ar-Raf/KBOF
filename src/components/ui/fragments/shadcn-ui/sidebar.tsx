...
function SidebarProvider({
  ...
  React.useEffect(() => {
    if (prevPathname.current !== pathname && isMobile && openMobile) {
      setOpenMobile(false)
    }
    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          className={cn(
            'group/sidebar-wrapper has-data-[variant=inset]:bg-sidebar flex min-h-lvh w-full',
            className,
          )}
          {...props}
        >
...
function Sidebar({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          'relative h-lvh w-(--sidebar-width) bg-transparent transition-[width] duration-200 ease-linear',
      ...
      <div
        className={cn(
          'fixed inset-y-0 z-10 hidden h-lvh w-(--sidebar-width) transition-[left,right,width] duration-200 ease-linear md:flex',
          side === 'left'
...
function SidebarRail({ className, ...props }: React.ComponentProps<'button'>) {
  ...
  return (
    <button
      ...
      className={cn(
        'hover:after:bg-sidebar-border  bg-transparent hove:bg-transparent absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:left-1/2 after:w-0.5 sm:flex',
...
function SidebarInset({ className, ...props }: React.ComponentProps<'main'>) {
  return (
    <main
      ...
      className={cn(
        'bg-background relative flex max-w-full min-h-lvh flex-1 flex-col',
