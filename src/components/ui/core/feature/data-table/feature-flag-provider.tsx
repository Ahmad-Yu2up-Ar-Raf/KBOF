...
import {
  ToggleGroup,
  ToggleGroupItem,
} from '@/components/ui/fragments/shadcn-ui/toggle-group'
import {
  Tooltip,
  TooltipContent,
  TooltipTrigger,
        ...
        >
          {flagConfig.featureFlags.map((flag) => (
            <Tooltip key={flag.value} delayDuration={700}>
              <ToggleGroupItem
                value={flag.value}
                className="primary-foreground space-nowrap px-3 text-xs data-[state=on]:bg-accent/70 data-[state=on]:hover:bg-accent/90 "
                asChild
              >
                <TooltipTrigger>
